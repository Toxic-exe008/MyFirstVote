...
// ═══════════════════════════════════════════════════════
var BASE = "https://myfirstvote-backend1.onrender.com";
...
function toast(msg, type) {
  ...
  t.style.cssText = "position:fixed;bottom:24px;right:24px;background:" +
    ...
    ";color:#0b1629;padding:12px 20px;border-radius:8px;font-weight:600;font-size:.85rem;z-index:9999;box-shadow:0 4px 20px rgba(0,0,0,.4)";
...
function loadDashboard() {
  Promise.all([apiReq("/api/admin/stats"), apiReq("/api/admin/audit-logs?limit=8")])
    .then(function(results) {
      ...
      if (stats.pendingReports > 0) { badge.textContent = stats.pendingReports; badge.style.display = "inline-block"; }
      else badge.style.display = "none";
      var logEl = document.getElementById("dashRecentLogs");
      if (!Array.isArray(logs) || !logs.length) { logEl.innerHTML = "<span>No activity yet.</span>"; return; }
      logEl.innerHTML = logs.map(function(l) {
        return "<div style='display:flex;gap:12px;padding:8px 0;border-bottom:1px solid #1e2435;align-items:center;font-size:.82rem'>" +
          ...
          "<span style='color:#475569;font-size:.72rem'>" + esc(l.entity) + "</span></div>";
      }).join("");
    }).catch(function(e) { console.error("Dashboard error:", e); });
}

function statCard(val, label, colour) {
  return "<div class='admin-stat-card'><div class='stat-val'" + (colour ? " style='color:" + colour + "'" : "") + ">" + val + "</div><div class='stat-label'>" + label + "</div></div>";
}
...
function loadCandidates() {
  ...
  apiReq("/api/candidates").then(function(data) {
    if (!Array.isArray(data) || !data.length) {
      tbody.innerHTML = "<tr><td colspan='6' style='text-align:center;color:#64748b;padding:32px'>No candidates found.</td></tr>"; return;
    }
    tbody.innerHTML = data.map(function(c) {
      return "<tr>" +
        ...
        "<td><span style='font-size:.82rem;color:" + ((c.criminalCases && c.criminalCases.length) ? "#f87171" : "#34d399") + "'>" + ((c.criminalCases && c.criminalCases.length) || 0) + "</span></td>" +
...
function editCandidate(id) {
  apiReq("/api/candidates").then(function(arr) {
    ...
    if (c) openCandidateModal(c); else toast("Candidate not found","error");
...
function addHistoryItem(h) {
  h = h || {};
  var div = document.createElement("div"); div.className = "dynamic-list-item";
  div.innerHTML = "<button class='remove-item' onclick='this.parentElement.remove()'>\u2715</button>" +
    "<div class='form-row'><div class='form-group' style='margin-bottom:8px'><label>Party</label><input type='text' class='h-party' value='" + esc(h.party||"") + "' placeholder='Party name'/></div>" +
    "<div class='form-group' style='margin-bottom:8px'><label>Position</label><input type='text' class='h-position' value='" + esc(h.position||"") + "' placeholder='e.g. MLA'/></div></div>" +
    "<div class='form-row'><div class='form-group' style='margin-bottom:8px'><label>From Year</label><input type='number' class='h-from' value='" + esc(h.fromYear||"") + "' placeholder='2019'/></div>" +
    "<div class='form-group' style='margin-bottom:8px'><label>To Year (0=Present)</label><input type='number' class='h-to' value='" + esc(h.toYear||"") + "' placeholder='2024 or 0'/></div></div>" +
    "<div class='form-group' style='margin-bottom:0'><label>Notes</label><input type='text' class='h-notes' value='" + esc(h.notes||"") + "' placeholder='Optional\u2026'/></div>";
...
function addCaseItem(cs) {
  ...
  var opts = statuses.map(function(s) { return "<option value='" + s + "'" + (cs.status===s?" selected":"") + ">" + s + "</option>"; }).join("");
  var div = document.createElement("div"); div.className = "dynamic-list-item";
  div.innerHTML = "<button class='remove-item' onclick='this.parentElement.remove()'>\u2715</button>" +
...
function addSourceItem(s) {
  ...
  var div = document.createElement("div"); div.className = "dynamic-list-item";
  div.innerHTML = "<button class='remove-item' onclick='this.parentElement.remove()'>\u2715</button>" +
    "<div class='form-row'><div class='form-group' style='margin-bottom:0'><label>Title</label><input type='text' class='src-title' value='" + esc(s.title||"") + "' placeholder='Source name'/></div>" +
    "<div class='form-group' style='margin-bottom:0'><label>URL *</label><input type='text' class='src-link' value='" + esc(s.link||"") + "' placeholder='https://\u2026'/></div></div>";
...
function saveCandidate() {
  var body = {
    ...
    politicalHistory: Array.from(document.querySelectorAll("#historyItems .dynamic-list-item")).map(function(el) {
      return { party: el.querySelector(".h-party").value.trim(), position: el.querySelector(".h-position").value.trim(),
               fromYear: parseInt(el.querySelector(".h-from").value)||null, toYear: parseInt(el.querySelector(".h-to").value)||0,
    ...
    criminalCases: Array.from(document.querySelectorAll("#caseItems .dynamic-list-item")).map(function(el) {
      return { status: el.querySelector(".cs-status").value, description: el.querySelector(".cs-desc").value.trim(),
  ...
  apiReq(path, { method: method, body: JSON.stringify(body) })
    .then(function(res) { if (res.error) throw new Error(res.error); toast(editingCandidateId ? "Candidate updated \u2713" : "Candidate created \u2713"); closeModal("candidateModal"); loadCandidates(); })
    .catch(function(e) { toast(e.message,"error"); });
}

function deleteCandidate(id, name) {
  confirm2("Delete \"" + name + "\"?", function() {
    apiReq("/api/admin/candidates/" + id, { method: "DELETE" })
      .then(function(r) { if (r.error) throw new Error(r.error); toast("Deleted"); loadCandidates(); })
      .catch(function(e) { toast(e.message,"error"); });
  });
}
...
function loadParties() {
  ...
  apiReq("/api/parties").then(function(data) {
    if (!Array.isArray(data) || !data.length) { tbody.innerHTML = "<tr><td colspan='5' style='text-align:center;color:#64748b;padding:32px'>No parties.</td></tr>"; return; }
    tbody.innerHTML = data.map(function(p) {
      return "<tr><td><span class='tbl-name'>" + esc(p.name) + "</span></td><td><span style='color:#f0c040;font-weight:700'>" + esc(p.abbreviation) + "</span></td>" +
        "<td><span class='tbl-badge " + (p.type||"").toLowerCase() + "'>" + esc(p.type) + "</span></td><td><span style='font-size:.82rem;color:#94a3b8'>" + (p.foundedYear||"\u2014") + "</span></td>" +
        "<td><div class='tbl-actions'><button class='btn-sm btn-edit' onclick='editParty(\"" + p._id + "\")'>\u270F Edit</button><button class='btn-sm btn-delete' onclick='deleteParty(\"" + p._id + "\",\"" + esc(p.name) + "\")'>\uD83D\uDDD1</button></div></td></tr>";
    }).join("");
  }).catch(function(e) { document.getElementById("partiesTableBody").innerHTML = "<tr><td colspan='5' style='color:#f87171;padding:20px'>" + esc(e.message) + "</td></tr>"; });
}

function openPartyModal(p) {
  p = p || null; editingPartyId = p ? p._id : null;
...
function editParty(id) {
  apiReq("/api/parties").then(function(data) {
    ...
    if (p) openPartyModal(p); else toast("Party not found","error");
  });
}
...
function addPartyElectionItem(e) {
  ...
  var div = document.createElement("div"); div.className = "dynamic-list-item";
  div.innerHTML = "<button class='remove-item' onclick='this.parentElement.remove()'>\u2715</button>" +
    "<div class='form-row'><div class='form-group' style='margin-bottom:8px'><label>Year</label><input type='number' class='pe-year' value='" + esc(e.year||"") + "' placeholder='2026'/></div>" +
    "<div class='form-group' style='margin-bottom:8px'><label>Election Name</label><input type='text' class='pe-election' value='" + esc(e.election||"") + "' placeholder='Maharashtra Assembly'/></div></div>" +
    "<div class='form-row'><div class='form-group' style='margin-bottom:8px'><label>Seats Won</label><input type='number' class='pe-won' value='" + esc(e.seatsWon!=null?e.seatsWon:"") + "'/></div>" +
    "<div class='form-group' style='margin-bottom:8px'><label>Total Seats</label><input type='number' class='pe-total' value='" + esc(e.totalSeats!=null?e.totalSeats:"") + "'/></div></div>" +
    "<div class='form-group' style='margin-bottom:0'><label>Notes</label><input type='text' class='pe-notes' value='" + esc(e.notes||"") + "'/></div>";
  document.getElementById("partyElectionItems").appendChild(div);
}

function saveParty() {
  var body = {
    name: document.getElementById("p-name").value.trim(), slug: document.getElementById("p-slug").value.trim(),
    abbreviation: document.getElementById("p-abbreviation").value.trim(), type: document.getElementById("p-type").value,
    foundedYear: parseInt(document.getElementById("p-foundedYear").value)||undefined, colour: document.getElementById("p-colour").value,
    foundedBy: document.getElementById("p-foundedBy").value.trim(), ideology: document.getElementById("p-ideology").value.trim(),
    symbol: document.getElementById("p-symbol").value.trim(), headquarters: document.getElementById("p-headquarters").value.trim(),
    description: document.getElementById("p-description").value.trim(),
    electionHistory: Array.from(document.querySelectorAll("#partyElectionItems .dynamic-list-item")).map(function(el) {
      return { year: parseInt(el.querySelector(".pe-year").value)||null, election: el.querySelector(".pe-election").value.trim(),
               seatsWon: parseInt(el.querySelector(".pe-won").value)||0, totalSeats: parseInt(el.querySelector(".pe-total").value)||undefined,
               notes: el.querySelector(".pe-notes").value.trim() };
    }).filter(function(e) { return e.year; })
  };
  if (!body.name||!body.slug||!body.abbreviation) { toast("Name, slug and abbreviation required","error"); return; }
  var path=editingPartyId?"/api/admin/parties/"+editingPartyId:"/api/admin/parties";
  var method=editingPartyId?"PUT":"POST";
  apiReq(path,{method:method,body:JSON.stringify(body)})
    .then(function(res){if(res.error)throw new Error(res.error);toast(editingPartyId?"Party updated \u2713":"Party created \u2713");closeModal("partyModal");loadParties();})
    .catch(function(e){toast(e.message,"error");});
}

function deleteParty(id, name) {
  confirm2("Delete party \""+name+"\"?",function(){
    apiReq("/api/admin/parties/"+id,{method:"DELETE"}).then(function(){toast("Party deleted");loadParties();}).catch(function(e){toast(e.message,"error");});
  });
}
...
function loadReports() {
  ...
  apiReq("/api/admin/reports" + (filter ? "?status=" + filter : "")).then(function(data) {
    if (!Array.isArray(data)||!data.length) { tbody.innerHTML="<tr><td colspan='6' style='text-align:center;color:#64748b;padding:32px'>No reports.</td></tr>"; return; }
    window.reports_cache = {};
    data.forEach(function(r) { window.reports_cache[r._id] = r; });
    tbody.innerHTML = data.map(function(r) {
      return "<tr>" +
        "<td><span class='tbl-report-id'>" + esc(r.reportId||"\u2014") + "</span></td>" +
        "<td style='font-size:.78rem;color:#64748b;white-space:nowrap'>" + fmtDate(r.createdAt) + "</td>" +
        "<td><div style='font-size:.82rem;color:#e2e8f0;font-weight:600'>" + esc(r.candidateName||"Unknown") + "</div></td>" +
        "<td><div class='report-msg-preview'>" + esc(r.message) + "</div></td>" +
        "<td><span class='tbl-badge " + esc(r.status) + "'>" + esc(r.status) + "</span></td>" +
        "<td><div class='tbl-actions'>" +
        "<button class='btn-sm btn-reply' onclick='openReportModal(window.reports_cache[\"" + r._id + "\"])'>\u21A9 Reply</button>" +
        "<button class='btn-sm btn-delete' onclick='deleteReport(\"" + r._id + "\")'>\uD83D\uDDD1</button>" +
        "</div></td></tr>";
    }).join("");
  }).catch(function(e) { document.getElementById("reportsTableBody").innerHTML = "<tr><td colspan='6' style='color:#f87171;padding:20px'>" + esc(e.message) + "</td></tr>"; });
}

function openReportModal(r) {
  if (!r) return; editingReportId = r._id;
  ...
  document.getElementById("reportModalContent").innerHTML =
    ...
    "<div style='font-size:.78rem;color:#64748b;margin-bottom:4px'>Report ID: <strong style='color:#f0c040;font-family:monospace'>" + esc(r.reportId||"\u2014") + "</strong></div>" +
    "<div style='font-size:.78rem;color:#64748b;margin-bottom:8px'>Candidate: <strong style='color:#e2e8f0'>" + esc(r.candidateName||"\u2014") + "</strong> &nbsp;\u00B7&nbsp; " + fmtDate(r.createdAt) + "</div>" +
    "<div style='background:#0f1117;border-left:3px solid #f0c040;padding:12px 14px;border-radius:4px;font-size:.88rem;color:#cbd5e1;margin-bottom:16px'>" + esc(r.message) + "</div></div>" +
    "<div class='form-group'><label>Status</label><select id='reply-status' class='admin-input' style='margin:0'>" + statusOpts + "</select></div>" +
    "<div class='form-group'><label>Admin Reply</label><textarea id='reply-text' class='admin-input' style='min-height:80px;resize:vertical;margin:0' placeholder='Optional reply\u2026'>" + esc(r.adminReply||"") + "</textarea></div>";
...
function submitReply() {
  ...
  apiReq("/api/admin/reports/" + editingReportId, { method: "PUT", body: JSON.stringify({ status: status, adminReply: reply }) })
    .then(function(res) { if (res.error) throw new Error(res.error); toast("Report updated \u2713"); closeModal("reportModal"); loadReports(); loadDashboard(); })
    .catch(function(e) { toast(e.message,"error"); });
}

function deleteReport(id) {
  confirm2("Delete report permanently?", function() {
    apiReq("/api/admin/reports/"+id,{method:"DELETE"}).then(function(){toast("Deleted");loadReports();}).catch(function(e){toast(e.message,"error");});
  });
}
...
function loadNotices() {
  ...
  apiReq("/api/admin/election-notices").then(function(data) {
    if (!Array.isArray(data)||!data.length) { tbody.innerHTML="<tr><td colspan='5' style='text-align:center;color:#64748b;padding:32px'>No notices.</td></tr>"; return; }
    tbody.innerHTML = data.map(function(n) {
      var ab = n.isActive ? "<span class='tbl-badge' style='background:rgba(16,185,129,.15);color:#34d399'>\u25CF Active</span>" : "<span class='tbl-badge' style='background:rgba(100,116,139,.15);color:#64748b'>Inactive</span>";
      return "<tr><td><span class='tbl-name'>" + esc(n.title) + "</span></td><td style='font-size:.82rem;color:#94a3b8'>" + esc(n.electionDate) + "</td>" +
        "<td style='font-size:.82rem;color:#94a3b8'>" + esc(n.votingStart) + " \u2013 " + esc(n.votingEnd) + "</td><td>" + ab + "</td>" +
        "<td><div class='tbl-actions'><button class='btn-sm btn-edit' onclick='editNotice(\"" + n._id + "\")'>\u270F Edit</button><button class='btn-sm btn-delete' onclick='deleteNotice(\"" + n._id + "\")'>\uD83D\uDDD1</button></div></td></tr>";
    }).join("");
  }).catch(function(e) { document.getElementById("noticesTableBody").innerHTML="<tr><td colspan='5' style='color:#f87171;padding:20px'>"+esc(e.message)+"</td></tr>"; });
}

function openNoticeModal(n) {
  n = n || null; editingNoticeId = n ? n._id : null;
...
function editNotice(id) {
  apiReq("/api/admin/election-notices").then(function(data) {
    ...
    if (n) openNoticeModal(n); else toast("Notice not found","error");
  });
}
...
function saveNotice() {
  ...
  if (!body.title||!body.electionDate) { toast("Title and election date required","error"); return; }
  ...
  apiReq(path,{method:method,body:JSON.stringify(body)})
    .then(function(res){if(res.error)throw new Error(res.error);toast("Notice saved \u2713");closeModal("noticeModal");loadNotices();})
    .catch(function(e){toast(e.message,"error");});
}

function deleteNotice(id) {
  confirm2("Delete notice?",function(){
    apiReq("/api/admin/election-notices/"+id,{method:"DELETE"}).then(function(){toast("Deleted");loadNotices();}).catch(function(e){toast(e.message,"error");});
  });
}
...
function loadAuditLog() {
  ...
  apiReq("/api/admin/audit-logs?limit=200").then(function(logs) {
    if (!Array.isArray(logs)||!logs.length) { container.innerHTML="<div style='padding:32px;text-align:center;color:#64748b'>No entries yet.</div>"; return; }
    container.innerHTML = logs.map(function(l) {
      return "<div class='audit-row'><span class='audit-ts'>" + fmtDate(l.createdAt) + "</span>" +
        ...
        "<span class='audit-entity'>" + esc(l.entity) + "</span></div>";
    }).join("");
  }).catch(function(e) { document.getElementById("auditLogBody").innerHTML="<div style='padding:20px;color:#f87171'>"+esc(e.message)+"</div>"; });
}

// ── MODAL CLOSE ───────────────────────────────────────────────────────────────
function closeModal(id) {
  document.getElementById(id).style.display = "none";
  editingCandidateId = editingPartyId = editingNoticeId = editingReportId = null;
}
